...
namespace WoWonder.Activities.GroupChat
{
    ...
    public class CreateGroupChatActivity : BaseActivity , MaterialDialog.ISingleButtonCallback, IActivityResultCallback
    {
        ...
        private void InitCropImage()
        {
            try
            {
                CropImage = RegisterForActivityResult(new CropImageContract(), this);
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
        ...
        private void OpenDialogGallery()
        {
            try
            {
                ...
                {
                    if (PermissionsController.CheckPermissionStorage("image") && CheckSelfPermission(Manifest.Permission.Camera) == Permission.Granted)
                    {
                        Methods.Path.Chack_MyFolder();
                        //var myUri = Uri.FromFile(new File(Methods.Path.FolderDiskImage, Methods.GetTimestamp(DateTime.Now) + ".jpg"));
                        var option = new CropImageContractOptions(null, new CropImageOptions()
