                ...
                if (viewHolder is HSharedFilesAdapterViewHolder holder)
                {
                    ...
                    switch (item.FileType)
                    {
                        case "Video":
                            {
                                ...
                                if (videoPlaceHolderImage == "File Dont Exists")
                                {
                                    ...
                                    Glide.With(ActivityContext).Load(photoUri).Apply(GlideImageLoader.GetOptions(ImageStyle.RoundedCrop, ImagePlaceholders.Drawable)).Into(holder.Image);
                                }
                                else
                                {
                        ...
                        case "Sounds":
                            ...
                            Glide.With(ActivityContext).Load(ActivityContext.GetDrawable(Resource.Drawable.Audio_File)).Apply(GlideImageLoader.GetOptions(ImageStyle.RoundedCrop,ImagePlaceholders.Drawable)).Into(holder.Image);
                            break;
                        case "File":
                            holder.PlayIcon.Visibility = ViewStates.Gone;
                            holder.TypeIcon.Visibility = ViewStates.Gone;

                            Glide.With(ActivityContext).Load(ActivityContext.GetDrawable(Resource.Drawable.Image_File)).Apply(GlideImageLoader.GetOptions(ImageStyle.RoundedCrop, ImagePlaceholders.Drawable)).Into(holder.Image);
                            break;
                    }
                }
            }
