...
#pragma warning restore CS0618
        ...
        private void IconView_Click()
        {
            try
            {
                ...
                try
                {
                    var item = TabbedMainActivity.GetInstance()?.ChatTab?.LastChatTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChat?.UserId == DataUser.UserId);
                    if (item?.LastChat != null && item.LastChat?.LastMessage.LastMessageClass.ToId == UserDetails.UserId && item.LastChat?.LastMessage.LastMessageClass.FromId != UserDetails.UserId)
                    {
                        ...
                        TabbedMainActivity.GetInstance()?.ChatTab?.LastChatTab?.MAdapter?.NotifyDataSetChanged();
                    }
                }
